...
let quotaCache = { ts: 0, left5h: 70, leftDay: 50, lastAttemptTs: 0 };
...
function fetchRealQuota() {
  ...
  // 5分钟刷新一次；且防抖20秒
  const stale = (now - quotaCache.ts) > 5 * 60 * 1000;
  const canTry = (now - (quotaCache.lastAttemptTs || 0)) > 20 * 1000;
  ...
  if (stale && canTry && !quotaFetchInFlight) {
    ...
    try {
      ...
      args.push('--message', '📊 session_status', '--json', '--thinking', 'off', '--timeout', '20');
      ...
      child.on('close', () => {
        try {
          ...
          const m5 = text.match(/5小时[^\n]*?(\d+)%/);
          const mDay = text.match(/每日[^\n]*?(\d+)%/);
